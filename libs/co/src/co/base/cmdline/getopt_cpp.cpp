...
int Getopt::Execute(const vector<string>& args, const char* optstring) {
  vector<char*> argv;
  for (size_t i = 0; i < args.size(); i++) {
    argv.push_back(const_cast<char*>(args[i].c_str()));
  }
  return Execute(args.size(), &argv[0], optstring);
}

int Getopt::Execute(int argc, char* argv[], const char* optstring) {
  int optchar = -1;
...
int Getopt::ExecuteLong(int argc, char* argv[], const char* optstring,
  const struct option* longopts, int* longindex)
{
  ...
  if (strlen(argv[optind]) < 3 || strncmp(argv[optind], "--", 2) != 0)
    return Execute(argc, argv, optstring);
